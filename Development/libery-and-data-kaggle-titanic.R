...
Age.bins <- cut(titanic_train$Age, c(0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50,60,70,80,90,100))
...
#Data Prep
# Add Survived column to test set so we can combine train and test sets
...
#Finally, we have two records with missing information for the port of 
#embarkation. I filter the data to find out which records these are, then 
#I determine if one port is more common than the rest.
...
#Now we return to those missing ages. The reason we didn’t worry about them earlier 
#is because I wanted to use the new title feature to help us impute the ages. 
#Many of the titles suggest a certain age group. For example, “master” was used 
#to refer to a young male. We’ll use a decision tree (rpart package) to estimate 
#the missing ages and then update the Age column with the new ages.
...
#Of course, the larger families were less common, which we can see by counting the 
#number of passengers that belonged to each family size grouping.
...
#Our AUC on our train set is 0.876, which is not bad at all. Let’s generate the 
#predictions for the test data and create a CSV to submit to Kaggle for scoring. 
#Notice that I use a cutoff probability value of 0.5 to classify whether a 
#passenger survives or not. This value is a parameter that we can adjust to 
#improve our fit, but I won’t get into that in this post. 
#A cutoff of 0.5 is oftentimes the default value used.
...
##################
#The second model choice, and the model for which I originally decided to write this 
#post, is the generalized additive model (GAM). GAMs are both interpretable and 
#flexible, which makes them very useful. I will build a fairly simple boosted GAM 
#using the train function in the caret package.
gbm.model <- train(Survived ~ Pclass + Sex + Age + SibSp + Parch + Family + Fare + Embarked + Title, 
                   data = titanic_train, 
                   method = "gbm", 
...
gbm.test <- predict(gbm.model, newdata = titanic_test, type = "raw")
gbm_submission <- data.frame(PassengerId = titanic_test$PassengerId, Survived = ifelse(logistic.test >= 0.5, 1, 0))
write.csv(gbm_submission, file = "liberty-and-data-gbm_submission.csv", row.names = FALSE)
#0.77990 with gbm
...
library(rpart)
library("rpart.plot")
rpart1 <- rpart(Survived ~ Pclass + Sex + Age + SibSp + Parch + 
                Family + Fare  + Title, 
                data = titanic_train,
                method="class",
                control = rpart.control(xval = 50, minbucket = 1, cp = 0) )
rpart1
rpart.plot(rpart1)

rpart1.test <- predict(rpart1, newdata = titanic_test)
rpart1_submission <- data.frame(PassengerId = titanic_test$PassengerId, Survived = ifelse(logistic.test >= 0.5, 1, 0))
write.csv(rpart1_submission, file = "liberty-and-data-rpart2_submission.csv", row.names = FALSE)
#0.77990
