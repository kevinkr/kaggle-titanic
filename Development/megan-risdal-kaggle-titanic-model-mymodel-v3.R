...
#KK Add another level of family size, medium
full$FsizeD[full$Fsize == 1] <- 'singleton'
full$FsizeD[full$Fsize == 2] <- 'small'
#full$FsizeD[full$Fsize > 1 & full$Fsize <= 7] <- 'small'
full$FsizeD[full$Fsize > 3] <- 'large'
...
full$Fare2 <- factor(full$Fare2)

#KK
#Disretize Age
full$AgeClass <- '60+'
full$AgeClass[full$Age < 60 & full$Age >= 50] <- '50-60'
full$AgeClass[full$Age < 50 & full$Age >= 40] <- '40-50'
full$AgeClass[full$Age < 40 & full$Age >= 30] <- '30-40'
full$AgeClass[full$Age < 30 & full$Age >= 20] <- '20-30'
full$AgeClass[full$Age < 20 & full$Age >= 10] <- '10-20'
full$AgeClass[full$Age < 10] <- '0-9'
full$AgeClass <- factor(full$AgeClass)
...
rf_model <- randomForest(factor(Survived) ~ Pclass + Sex + AgeClass + SibSp + Parch + 
                           Fare2 + Embarked + Title + 
                           FsizeD + Child + Mother,
                         data = train)
...
write.csv(solution, file = 'megan-risdal-orig-Solution-kk-v3.csv', row.names = F)
